...
a=np.array([-1,1,-1])+np.random.rand(parameter.n[0],parameter.dim)
b=np.array([0,-1,1.5]) +np.random.rand(parameter.n[1],parameter.dim)
c=np.array([1,0,1]) +np.random.rand(parameter.n[2],parameter.dim)
d=np.array([1,-1,0])+np.random.rand(parameter.n[3],parameter.dim)
...
df = pd.DataFrame(dataset,columns=['x', 'y', 'z', 'label'])
df = shuffle(df)
df.index = range(len(df))

#randomly pick up four scatter points to initialize kmean.
kmean=df.loc[np.random.randint(0,np.sum(parameter.n),parameter.guess_nk),['x','y','z']]
...
ax.scatter(kmean.loc[:,'x'], kmean.loc[:,'y'], kmean.loc[:,'z'], c='black')
...
for i, m in [(0,'r'),(1,'y'),(2,'g'),(3,'b')]:
    ax.scatter(df.loc[np.where(df['label']==i)]['x'], 
                      df.loc[np.where(df['label']==i)]['y'],
                      df.loc[np.where(df['label']==i)]['z'], c=m)
ax.scatter(kmean.loc[:,'x'], kmean.loc[:,'y'], kmean.loc[:,'z'], c='black')
ax.set_xlabel('X Label')
ax.set_ylabel('Y Label')
ax.set_zlabel('Z Label')
